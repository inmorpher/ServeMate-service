...
describe('PaymentService', () => {
	...
	describe('createPayment', () => {
		...
		it('should throw error when trying to add processed items', async () => {
			...
			await expect(paymentService.createPayment(1, [201], [101])).rejects.toThrow(
		...
		it('should handle update failure for order items', async () => {
			...
			await expect(paymentService.createPayment(1, [201], [101])).rejects.toThrow(
				'Failed to update order items status'
			);
		});
	});
	...
	describe('completePayment', () => {
		...
		it('should handle database error during order status check', async () => {
			jest
				.spyOn(paymentService as any, 'checkOrderPaymentCompletion')
	...
	describe('refundPayment', () => {
		...
		it('should throw error when payment already refunded', async () => {
			...
			await expect(paymentService.refundPayment(1, 'Test')).rejects.toThrow(
				'Payment already refunded'
			);
		});

		it('should throw error when refund reason is empty', async () => {
			await expect(paymentService.refundPayment(1, '')).rejects.toThrow(
	...
	describe('checkOrderPaymentCompletion', () => {
		it('should return true when all items are paid', async () => {
			mockPrisma.order.findUnique.mockResolvedValue({
				foodItems: [{ paymentStatus: PaymentState.PAID }, { paymentStatus: PaymentState.PAID }],
				drinkItems: [{ paymentStatus: PaymentState.PAID }],
			});
		...
		it('should return false when some items are not paid', async () => {
			mockPrisma.order.findUnique.mockResolvedValue({
				foodItems: [{ paymentStatus: PaymentState.PAID }, { paymentStatus: PaymentState.PENDING }],
				drinkItems: [{ paymentStatus: PaymentState.PAID }],
			});
		...
		it('should throw error when order not found', async () => {
			mockPrisma.order.findUnique.mockResolvedValue(null);
			await expect((paymentService as any).checkOrderPaymentCompletion(1)).rejects.toThrow(
				'Order not found'
			);
		});
	...
	describe('AbstractPaymentService', () => {
		describe('calculatePaymentAmount', () => {
			it('should calculate total amount correctly', () => {
				const items = [{ finalPrice: 10 }, { finalPrice: 20 }, { finalPrice: 30 }];
		...
		describe('validateItems', () => {
			...
			it('should validate food items with invalid status', () => {
				expect(() => (paymentService as any).validateItems([], [2], mockOrderItems)).toThrow(
					'Invalid items selected or cannot add processed items to another payment'
				);
			});

			it('should validate drink items with invalid status', () => {
				expect(() => (paymentService as any).validateItems([5], [], mockOrderItems)).toThrow(
					'Invalid items selected or cannot add processed items to another payment'
				);
			});
