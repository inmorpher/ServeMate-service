...
describe('AuthenticationController', () => {
	...
	it('logs in a user and returns tokens', async () => {
		...
		const controller = new AuthenticationController(logger, tokenService, userService);
		const user = {
			id: 1,
			name: 'Alice',
	...
	it('refreshes tokens when a valid refresh token is supplied', async () => {
		...
		const req = createMockRequest({ body: { refreshToken: 'old-refresh-token' } });
		const res = createMockResponse();
		const next = createMockNext();

		await controller.refreshToken(req as any, res, next);

		expect(tokenService.refreshToken).toHaveBeenCalledWith('old-refresh-token', userService);
		expect(res.status).toHaveBeenCalledWith(200);
		expect(res.json).toHaveBeenCalledWith({
			accessToken: 'new-access-token',
			refreshToken: 'new-refresh-token',
	...
	it('returns the authenticated user from me()', async () => {
		...
		const tokenService = createTokenServiceMock();
		const controller = new AuthenticationController(logger, tokenService, userService);
		(userService.findUserById as jest.Mock).mockResolvedValue({ id: 1, name: 'Alice' });
		const req = createMockRequest({
	...
	it('returns unauthorized from me() when there is no authenticated user', async () => {
		...
		expect(res.status).toHaveBeenCalledWith(401);
		expect(res.json).toHaveBeenCalledWith({ message: 'Пользователь не аутентифицирован' });
	});
});