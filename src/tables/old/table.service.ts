  ...
  any> {
    try {
      ...
      const where = {
        ...
        ...(status && {
          status: { equals: status.toUpperCase() as TableCondition },
        }),
    ...
    try {
      ...
      if (existingTable) {
        throw new HTTPError(
          400,
          'TableService',
          'Table with the same table number already exists'
        );
      }
