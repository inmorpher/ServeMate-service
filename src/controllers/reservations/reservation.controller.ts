...
export class ReservationController extends BaseController {
	...
	async createReservation(
		...
		try {
			const reservation = await this.reservationService.createReservation(getValidatedBody(req));
	...
	async getAllReservations(
		...
		try {
			const reservations = await this.reservationService.getReservationsByCriteria(getValidatedQuery(req));
	...
	async updateReservationTime(
		...
		try {
			const updatedReservation = await this.reservationService.updateReservationTime(
				Number(getValidatedParams(req).id),
				new Date(getValidatedBody(req).time)
			);
			this.ok(res, updatedReservation);
		} catch (error) {
			next(error);
	...
	async updateReservationGuestInfo(
		...
		try {
			const updatedReservation = await this.reservationService.updateReservationGuestInfo(
				Number(getValidatedParams(req).id),
				getValidatedBody(req).guestInfo
			);
			this.ok(res, updatedReservation);
		} catch (error) {
			next(error);
	...
	async deleteReservation(
		...
		try {
			await this.reservationService.deleteReservation(Number(getValidatedParams(req).id));
