...
export class AuthenticationController extends BaseController {
  private authCache = new NodeCache({ stdTTL: 300, checkperiod: 60 });
  constructor(
  ...
  async logout(
    ...
    try {
      ...
      const refreshToken = (
        req as Request & { cookies?: { refreshToken?: string } }
  ...
  async validateUser(user: UserCredentials): Promise<ValidatedUserData> {
    ...
    const userRole: UserRole =
      UserRole[existingUser.role as keyof typeof UserRole];
