  ...
  protected bindRoutes() {
    ...
    routes.forEach((route: RouteDefinition) => {
      ...
      if (route.middlewares && route.middlewares.length > 0) {
        const middlewares = route.middlewares.map((m: any) =>
          m.execute.bind(m)
        );
        ...
        this.logger.debug(
          `Registered route: ${route.method.toUpperCase()} ${prefix + route.path}`
        );
      }
    });
  }

  protected getValidated<TParams = unknown, TQuery = unknown, TBody = unknown>(
