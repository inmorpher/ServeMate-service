...
async function generateOrders(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    orders.push({
      ...
      comments:
        Math.random() > 0.7 ? faker.commerce.productDescription() : undefined,
      discount,
      tip: faker.number.float({
        min: 0,
        max: totalAmount * 0.2,
        fractionDigits: 2,
      }),
  ...
  try {
    ...
    for (const order of orders) {
      ...
      for (let i = 0; i < foodItemsCount; i++) {
        ...
        const guestNumber = faker.number.int({
          min: 1,
          max: order.guestsCount,
        });
        ...
        if (Math.random() > 0.7) {
          ...
          const selectedAllergies = faker.helpers.arrayElements(
            allergies,
            allergyCount
          );
          ...
          for (const allergy of selectedAllergies) {
            try {
              ...
              console.log(
                `Allergy ${allergy} already exists for food item ${orderFoodItem.id}`
              );
            }
          }
        }
        ...
        if (Math.random() > 0.8) {
          ...
          for (const allergy of selectedAllergies) {
            try {
              ...
              // Skip if allergy already exists for this item
              console.log(
                `Allergy ${allergy} already exists for drink item ${orderDrinkItem.id}`
              );
            }
          }
        }
    ...
    console.log(
      `Successfully added ${ordersCount} random orders (from Jan 19 to Feb 19) with allergies to the database.`
    );
  } catch (error) {
